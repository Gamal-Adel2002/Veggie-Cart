...
echo "Secret value: (see below — copy the ENTIRE single-line base64 string)"
echo ""
# -w 0 forces single-line output on Linux; macOS `base64` is single-line by default
...
echo "──────────────────────────────────────────────"
echo ""
echo "After adding the secrets to Replit, build your release AAB with a single command:"
echo ""
echo "  cd artifacts/freshveg-mobile"
echo "  flutter build appbundle --dart-define=FLUTTER_API_BASE_URL=https://your-app.replit.app"
echo ""
echo "(Gradle will automatically decode the keystore and configure signing at build time.)"
echo ""
