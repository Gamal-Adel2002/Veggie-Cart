...
export default function Suppliers() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      toast({ title: 'Name required', description: 'Supplier name cannot be empty.', variant: 'destructive' });
      return;
    }
    setSaving(true);
    try {
    ...
    <AdminLayout>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Building2 className="w-5 h-5 text-primary" /> Suppliers
        </h2>
        <Button onClick={openNew}><Plus className="w-4 h-4 me-2" /> Add Supplier</Button>
      </div>

      <div className="relative mb-4">
        <Search className="absolute start-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, phone, address..." className="ps-9" />
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={open => { if (!open) setDialogOpen(false); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editTarget ? `Edit — ${editTarget.name}` : 'Add Supplier'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 pt-1">
            ...
            <div>
              ...
              <Input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="e.g. 01012345678" />
            </div>
            <div>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : editTarget ? 'Save Changes' : 'Create Supplier'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete <strong>{deleteTarget?.name}</strong>? This will also delete all their purchase orders. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
