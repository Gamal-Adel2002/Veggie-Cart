  ...
  return (
    <AdminLayout>
      {showBanner && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-amber-800 mb-1">{t('adminLowStockAlert')}</p>
            <ul className="space-y-0.5">
              {lowStockProducts.map(p => (
                <li key={p.id} className="text-sm text-amber-700">
                  <span className="font-medium">{p.name}</span>
                  {p.quantity !== null && p.quantity !== undefined && (
                    <> — <span className="font-bold">{t('adminLowStockRemaining')(p.quantity, p.unit)}</span>
                      {p.quantityAlert !== null && p.quantityAlert !== undefined && (
                        <span className="text-amber-500"> {t('adminLowStockThreshold')(p.quantityAlert, p.unit)}</span>
                      )}
                    </>
                  )}
                </li>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('adminTotalRevenue')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('adminTotalOrders')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('adminPendingOrders')}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('adminCustomers')}</CardTitle>
      ...
      <div className="bg-card border border-border p-8 rounded-3xl min-h-[400px] flex items-center justify-center">
        <p className="text-muted-foreground">{t('adminSelectSidebar')}</p>
      </div>
    </AdminLayout>
  );
}
